...
function drawBoard(board) {
    ...
    for (let rowIndex = 0; rowIndex < 3; rowIndex++) {
        for (let columnIndex = 0; columnIndex < 3; columnIndex++) {
            ...
            neoImage.src = "Keanu.png"

            const morpheusImage = document.createElement("img");
            morpheusImage.src = "Morpheus.png"

            const cellText = board[rowIndex][columnIndex] === "Morpheus" ? morpheusImage : neoImage;
...
function isValidColumn(columnArray) {
    return isValidRowOrColumn(columnArray) && columnArray.every(function (item) { return ["Morpheus", "Neo", null].includes(item); });
}
...
function positionClick(rowIndex, columnIndex, event) {
    if (gameOver === false) {
        ...
        if (!isValidRowOrColumn(board) || !board.every(isValidColumn)) {
            throw "Expecting 'getBoard' to return a 2d array where all values match are null or one of the strings 'Morpheus' or 'Neo'. Actually received: " + JSON.stringify(board);
        }
        drawBoard(board);
